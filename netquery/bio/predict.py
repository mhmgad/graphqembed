...
from netquery.train_helpers import run_train, run_eval, run_predict
# from netquery.utils import predict
...
log_file = args.log_dir + "/{data:s}-{depth:d}-{embed_dim:d}-{lr:f}-{decoder:s}-{inter_decoder:s}.log_predict".format(
...
enc_dec.load_state_dict(torch.load(saved_model_filename))
enc_dec.eval()
...
#run_eval(enc_dec, test_queries, 1, logger, by_type=False)
# enc_dec(test_queries)
preds=run_predict(enc_dec,test_queries)

with open("predictions.out", "wb" ) as out_file:
        for query_type in preds:
            for q in preds[query_type]:
                out_file.write(query_type+'\t'+str(q[0].query_graph)+'\t'+str(q[1])+'\t'+str(q[2])+'\n')
